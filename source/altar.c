...
void update_barrier(entity_t *barrier) {
	if (!barrier->door_closed) {
		...
		obj_set_attr(&obj_mem[reserve_obj()],
			...
			barrier->tid | ATTR2_PRIO(0) | ATTR2_PALBANK(5));
...
void altars_update() {
	...
	// if (entity_collide(&altar_jetpack, &player)) {
		
	// }
	// if (entity_collide(&altar_shield, &player)) {
